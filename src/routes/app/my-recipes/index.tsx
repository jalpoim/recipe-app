...
function RecipeListSkeleton() {
  return (
    <div className="space-y-3">
      {[0, 1, 2, 3].map((i) => (
        <div key={i} className="rounded-2xl bg-white border border-[#F0F0EE] p-4 animate-pulse">
    ...
    <div className="min-h-screen bg-[#FAFAF8] pb-24">
      ...
      <div className="sticky top-0 z-10 bg-[#FAFAF8] border-b border-[#F0F0EE]">
        <div className="max-w-md mx-auto px-4 py-3 flex items-center justify-between">
          <h1 className="text-base font-semibold text-[#1A1A1A]">{t('myRecipes.title')}</h1>
          <Link
            to="/app/settings"
      ...
      <div className="max-w-md mx-auto px-4 py-6 space-y-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-lg font-bold text-[#1A1A1A] truncate">{displayName}</p>
            {username && <p className="text-sm text-[#9CA3AF]">@{username}</p>}
            {bio && <p className="text-sm text-[#6B7280] mt-0.5 line-clamp-2">{bio}</p>}
          </div>
          <Link
            to="/app/settings"
        ...
        {tab === 'created' && (
          <div>
            {createdLoading ? (
              ...
              <p className="text-sm text-[#9CA3AF] text-center py-8">{t('myRecipes.noCreated')}</p>
            ) : (
              <div className="space-y-3">
        ...
        {tab === 'saved' && (
          <div>
            {savedLoading ? (
              ...
              <p className="text-sm text-[#9CA3AF] text-center py-8">{t('myRecipes.noSaved')}</p>
            ) : (
              <div className="space-y-3">
