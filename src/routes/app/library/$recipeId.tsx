...
function scaleIngredient(
  ...
  const { value: converted, unit: displayUnit } = convertUnit(
    scaled,
    ing.unit ?? "",
    measurementSystem,
  );
    ...
    >
      ...
      <div className="flex items-center px-4 pt-2 pb-1 shrink-0 gap-2">
        <button
          ...
          aria-label={
            expanded ? t("cooking.collapseDrawer") : t("cooking.expandDrawer")
          }
...
export const Route = createFileRoute("/app/library/$recipeId")({
  ...
  validateSearch: (search) => ({
    ...
    planItemId:
      typeof search.planItemId === "string" ? search.planItemId : undefined,
  }),
...
function RecipeDetailPage() {
  ...
  const isLiked = interactions.some(
    (i) => i.recipe_id === recipe.id && i.type === "like",
  );
  const isSaved = interactions.some(
    (i) => i.recipe_id === recipe.id && i.type === "save",
  );
  ...
  const myCookCount =
    cookCounts?.find((c) => c.recipe_id === recipe.id)?.count ?? 0;

  const logCookMutation = useMutation({
    mutationFn: () =>
      logRecipeCooked({ data: { recipeId: recipe.id, source: "manual" } }),
    onSuccess: (row) => {
  ...
  const saveMultMutation = useMutation({
    mutationFn: (mult: number) =>
      Promise.all([
        updatePlanItemMultiplier({
          data: { planItemId: search.planItemId!, multiplier: mult },
        }),
        upsertUserRecipePreference({
          data: { recipeId: recipe.id, servings: mult },
        }),
      ]),
  ...
  const cal = scaledMacro(
    recipe.calories,
    recipe.servings,
    recipe.macros_total,
    multiplier,
  );
  const pro = scaledMacro(
    recipe.protein,
    recipe.servings,
    recipe.macros_total,
    multiplier,
  );
  const carb = scaledMacro(
    recipe.carbs,
    recipe.servings,
    recipe.macros_total,
    multiplier,
  );
  const fat = scaledMacro(
    recipe.fat,
    recipe.servings,
    recipe.macros_total,
    multiplier,
  ...
  const ingredientSections: {
    label: string | null;
    items: typeof recipe.recipe_ingredients;
  }[] = [];
  ...
  for (const ing of recipe.recipe_ingredients) {
    ...
    if (!sectionMap.has(key)) {
      const s = {
        label: ing.section_label,
        items: [] as typeof recipe.recipe_ingredients,
      };
  ...
  return (
    <>
      <div
        ...
        style={{
          paddingBottom: isCooking
            ? "calc(256px + 3.25rem + env(safe-area-inset-bottom) + 1.5rem)"
          ...
          <div className="px-4 pt-5 space-y-5">
                    ...
                    >
                      {label && (
                        <div className="px-4 py-2 border-b border-[#F3F4F6] flex items-center gap-2">
                          ...
                          <span className="text-[10px] text-[#9CA3AF] border border-[#E5E7EB] rounded-full px-1.5 py-0.5">
                            {t("recipe.optional")}
                          </span>
                        </div>
                      )}
                      <div className="divide-y divide-[#F3F4F6]">
                        {items.map((ing) => (
                          ...
                          >
                            <span
                              className={`text-sm ${ing.is_optional && !label ? "text-[#6B7280]" : "text-[#1A1A1A]"}`}
                            >
                              {scaleIngredient(
                                ing,
                                multiplier,
            ...
            <div
              style={{
                ...
                transition:
                  "grid-template-rows 350ms ease-out, opacity 300ms ease-out",
              ...
              <div className="grid grid-cols-4 gap-2">
                {[
                  ...
                  >
                    ...
                    <div className="text-[9px] text-[#9CA3AF] font-medium">
                      {unit}
                    </div>
                  </div>
                ))}
                ...
                <ol className="space-y-4">
                  {recipe.recipe_steps.map((step, i) => (
                    <li key={step.id} className="flex gap-3">
                      <span
                        className={`shrink-0 w-6 h-6 rounded-full text-xs font-bold flex items-center justify-center mt-0.5 transition-colors duration-200 ${
                          isCooking && i < completedUpToStep
                            ? "bg-[#F4623A] text-white"
                            : "bg-[#FEE9E1] text-[#D94F2B]"
                        }`}
                      >
                        {i + 1}
                      </span>
                      <p
                        className={`text-sm leading-relaxed transition-opacity duration-200 ${isCooking && i < completedUpToStep ? "text-[#9CA3AF]" : "text-[#374151]"}`}
                      >
                        {step.text}
                      </p>
                    </li>
            ...
            {!isCooking && (
              <div className="relative pt-2 pb-2">
                ...
                >
                  ...
                  {myCookCount > 0 || lastCookLogId
                    ? t("recipe.logCookedAgain")
                    : t("recipe.logCooked")}
                  <AnimatePresence>
                    {justCooked && (
                      <motion.span
