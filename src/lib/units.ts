...
const COUNT_UNITS = new Set([
  ...
  "fl oz",
  "oz",
  "lb", // user-authored imperial pass-through
]);
...
// Returns grams equivalent for a qty+unit pair. Returns null for count units where
// a gram conversion is ambiguous (clove, slice, etc.) — callers should skip those.
export function convertToGrams(qty: number, unit: string): number | null {
  if (!unit || unit === "g") return qty;
  if (unit === "kg") return qty * 1000;
  if (unit === "ml") return qty; // water approximation, good enough for nutrition
  ...
  if (unit === "fl oz") return qty * 29.574;
  return null; // count units: unit, slice, clove, pinch, bunch, handful, sheet, can, sachet
}
