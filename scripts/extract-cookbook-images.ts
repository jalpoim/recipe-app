...
function listUniqueImages(pdfPath: string, minPage?: number): ImageInfo[] {
  const raw = execSync(`pdfimages -list "${pdfPath}" 2>/dev/null`, {
...
// Extract raw embedded images from a PDF page range using pdfimages -j.
// These are the actual bitmap objects stored in the PDF — they do NOT include
// any PDF text layers, so food photos come out clean even when the page layout
// has text/titles overlaid on top of the image.
// Falls back gracefully: returns [] if nothing qualifies.
async function extractEmbeddedImages(
  pdfPath: string,
  fromPage: number,
  toPage: number,
  minPx: number = MIN_DIMENSION,
): Array<{ buf: Buffer; page: number }> {
  const tmpDir = os.tmpdir();
  const prefix = path.join(tmpDir, `pdfimg-${process.pid}-${Date.now()}`);

  try {
    // Get full image list with metadata so we can match extracted files to pages/dimensions
    const listOut = execSync(
      `pdfimages -list -f ${fromPage} -l ${toPage} "${pdfPath}" 2>/dev/null`,
      { maxBuffer: 4 * 1024 * 1024 },
    ).toString();
    ...
    // Build a map from sequential extraction index → {page, qualifies}
    const imageMap = new Map<number, { page: number; qualifies: boolean }>();
    let extractionIdx = 0;
    for (const line of lines) {
      const cols = line.trim().split(/\s+/);
      if (cols.length < 13) continue;
      ...
      const qualifies =
        ...
        width >= minPx &&
        height >= minPx &&
        height / width >= MIN_PORTRAIT_RATIO;
      imageMap.set(extractionIdx, { page, qualifies });
      extractionIdx++;
    }

    if (![...imageMap.values()].some((v) => v.qualifies)) return [];

    // Extract images from the page range (forces JPEG; other formats → ppm/pbm/pgm)
    execSync(
      `pdfimages -j -f ${fromPage} -l ${toPage} "${pdfPath}" "${prefix}" 2>/dev/null`,
      { maxBuffer: 50 * 1024 * 1024, timeout: 30000 },
    );

    const result: Array<{ buf: Buffer; page: number }> = [];
    const possibleExts = [".jpg", ".ppm", ".png", ".pbm", ".pgm"];

    for (const [idx, meta] of imageMap) {
      if (!meta.qualifies) continue;
      const padded = String(idx).padStart(3, "0");
      for (const ext of possibleExts) {
        const filePath = `${prefix}-${padded}${ext}`;
        if (fs.existsSync(filePath)) {
          try {
            // Normalise to JPEG so sharp/Vision can always read it
            const raw = fs.readFileSync(filePath);
            const jpeg = await sharp(raw).jpeg({ quality: 90 }).toBuffer();
            result.push({ buf: jpeg, page: meta.page });
          } catch {
            // Unsupported format (e.g. CMYK JPEG, JBIG2) — skip this image
          }
          break;
        }
      }
...
function findRecipeTitlePages(
  ...
  const normalizedNames = recipes.map((r) => ({
    id: r.id,
    name: r.name,
  ...
  for (const r of normalizedNames) {
    ...
    if (overridePage !== undefined) {
      ...
      if (verbose)
        console.log(`  p${overridePage} → "${r.name}" (manual override)`);
    }
  }

  console.log(
    `  Scanning ${totalPages - minPage + 1} pages for recipe titles (${overriddenIds.size} overridden)...`,
  );
  ...
  for (let page = minPage; page <= totalPages; page++) {
    ...
    for (const r of normalizedNames) {
      ...
      if (textScore > 0) {
        ...
        const hasNearbyImage = [0, 1, 2, 3].some(
          (offset) =>
            portraitImagePages.has(page + offset) ||
            portraitImagePages.has(page - offset),
        );
        scores.push({
          id: r.id,
  ...
  for (const r of normalizedNames) {
    ...
    const candidates = scores
      ...
      .sort((a, b) => {
        ...
        if (a.hasNearbyImage !== b.hasNearbyImage)
          return a.hasNearbyImage ? -1 : 1;
    ...
    if (pick) {
      ...
      if (verbose) {
        ...
        console.log(
          `  p${pick.page} → "${r.name}" (${flag}, ${matchesPerPage.get(pick.page)} page-matches)`,
        );
      }
    } else if (verbose) {
...
async function selectBestFoodPage(
  ...
  for (let i = 0; i < resized.length; i++) {
    content.push({
      type: "text",
      text: `Page ${i + 1} (PDF page ${resized[i].page}):`,
    });
    content.push({
      type: "image",
      source: {
  ...
  content.push({
    type: "text",
    text:
      `These are consecutive pages from a cookbook for the recipe "${recipeName}". ` +
      `Which page (if any) is the best hero/thumbnail for this recipe in a meal prep app? ` +
      `Prefer: a clean food-only photo of the final plated dish with no text visible on the image itself, ` +
      `complete in frame, good lighting, no faces. Hands holding the finished dish are fine. ` +
      `SKIP any of the following — return 0 if all pages fall into these categories: ` +
      `(1) pages where any readable text, title, caption, ingredient list, or recipe instructions appear overlaid ON TOP of the food photo; ` +
      `(2) full PDF page renders that combine a food photo with columns of text or paragraphs beside or around it; ` +
      `(3) intro, title, or marketing pages where a food photo is used as a background image with text overlay; ` +
      `(4) mostly-text pages with little or no food visible; ` +
      `(5) step-by-step prep shots or mid-cook shots; ` +
      `(6) pages where the dish is cut off at the edges. ` +
      `A page with ANY text visible on or overlaid on the food photo must be skipped. ` +
...
async function fetchRecipes(
  ...
  if (name_language === "en") {
    ...
    const enMap = new Map(
      ((rtrans ?? []) as Array<{ recipe_id: string; name: string }>).map(
        (t) => [t.recipe_id, t.name],
      ),
    );
    recipes = recipes.map((r) => ({
      id: r.id,
      name: enMap.get(r.id) ?? r.name,
    }));
  }

  // Filter to only recipes listed in the JSON file (disambiguates overlapping time windows)
  if (jsonPath && fs.existsSync(jsonPath)) {
    const jsonRecipes = JSON.parse(fs.readFileSync(jsonPath, "utf8")) as Array<{
      name: string;
    }>;
...
async function processPdfSource(source: PdfSource) {
  ...
  console.log(
    `  PDF: ${totalPages} total pages (scanning from page ${minPage})`,
  );
  ...
  console.log(
    `  ${recipeImages.length} portrait image objects found (for TOC filtering)`,
  );

  // Find which page each recipe title appears on
  const titlePages = findRecipeTitlePages(
    source.pdfPath,
    totalPages,
    recipes,
    minPage,
  ...
  console.log(
    `  Matched ${titlePages.size}/${recipes.length} recipes to pages`,
  );
  ...
  const sorted = Array.from(pageRanges.entries()).sort(
  ...
  if (DRY_RUN) {
    ...
    for (const [id, range] of sorted) {
      ...
      console.log(
        `  p${range.from}–${range.from + nPages - 1} (of p${range.from}–${range.to}) "${recipe.name}"`,
      );
    }
    return;
  }
  ...
  for (const [id, range] of sorted) {
    ...
    // Strategy 1: extract raw embedded images (no PDF text layer → clean photos)
    let embeddedImages = await extractEmbeddedImages(
      source.pdfPath,
      range.from,
      range.to,
      minPx,
    ...
    if (candidates.length === 0) {
      for (
        let p = range.from;
        p <= range.to && candidates.length < MAX_RENDER_PAGES;
        p++
      ) {
        const buf = renderPageToBuffer(source.pdfPath, p);
        if (buf) candidates.push({ page: p, buf });
      }
    }

    if (candidates.length === 0) {
      console.log(
        `  ⚠ No images found for "${recipe.name}" (p${range.from}–${range.to})`,
      );
      noPage++;
      continue;
    }
  ...
  console.log(
    `  ${recipes.length - titlePages.size} recipes unmatched (no title page found)`,
  );
}
...
async function main() {
  ...
  if (sources.length === 0) {
    console.error(
      `Unknown --pdf filter: ${PDF_FILTER}. Use "cooking" or "joe".`,
    );
    process.exit(1);
  }
